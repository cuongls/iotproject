...
namespace IOTProject.Tests
{
    public abstract class IOTProjectTestBase : AbpIntegratedTestBase<IOTProjectTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected IOTProjectTestBase()
        {
        ...
        protected void UsingDbContext(Action<IOTProjectDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<IOTProjectDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<IOTProjectDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<IOTProjectDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<IOTProjectDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<IOTProjectDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<IOTProjectDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<IOTProjectDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<IOTProjectDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<IOTProjectDbContext, Task<T>> func)
        {
            T result;
